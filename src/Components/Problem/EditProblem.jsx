    ...
    useEffect(() => {
        const SetDefaultValues = () => {
            if (success) {
                ...
                setCategory(problems.category)
                setTitle(tit);
                setDescription(problems.description);
                setMoney(problems.financial_amount)
                // https://biglybigly.iran.liara.run/${problems.main_image}
                fetch(`https://biglybigly.iran.liara.run/${problems.main_image}`, {
                    method: 'GET',
                })
